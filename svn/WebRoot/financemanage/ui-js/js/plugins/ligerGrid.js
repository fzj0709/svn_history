...
(function ($)
{
    ...
    {
    	//options为各自页面展示用到的相关属性信息对象
    	//this执行当前调用ligerGrid方法的jQuery对象，通常是一个div对象
    	options.eId =  this.attr("id");
        var manager = $.ligerui.run.call(this, "ligerGrid", arguments);//执行了base.js中的run方法
        ...
        //------------------------------------------------------------------start----------------------------------------------------------
		if(dragAble){
			//鼠标点击开始拖动时注册事件，记录鼠标位置
	        var tableId = this.attr("id");	//页面调用ligerGrid方法绘制表格的div的id
	        ...
	        //bodyHeight代表了整个页面的高度
	        var heightDataGrid = parseInt($("#"+tableId).css("height").replace("px",""));
	        //heightDataGrid代表了当前展示列表的高度
	        jQuery("#main").css("height",bodyHeight-heightDataGrid-12);//列表中通常都有新增或修改功能这里控制了新增页面或修改页面展示的高度
	        $("#fgt").bind("mousedown",function(event){
				  ...
				  var dragData = {x : oEvent.clientX, y : oEvent.clientY};//实时记录鼠标的起始点坐标
				  var startData = {x : oEvent.clientX, y : oEvent.clientY};//记录鼠标的起始点坐标
				  ...
				  function mouseup(){
					   var oEvent = window.event ? window.event : event;
					   var opY = parseFloat(dragData.y)-parseFloat(startData.y);//纵向移动的距离
    ...
    $.ligerDefaults.Grid = {
        ...
        page: 1,                                //默认当前页 
        pageSize: 3,                           //每页默认的结果数
        pageSizeOptions: [3,10,20,50,100],  //可选择设定的每页结果数
        parms: [],                         //提交到服务器的参数
        columns: [],                          //数据源
        ...
        cssClass: null,                    //类名
        root: 'getResult',                       //数据源字段名
        record: 'resultCount',                     //数据源记录数字段名
        ...
        allowUseCtrlKey:false,    			//是否启用ctrl键控制多选
        ...
        frozenRownumbers: true,   //让行号列自适应数据列高度               //行序号是否在固定列中
        ...
        bottomBar:null,                          //底部工具栏添加
        headerImg: null,                        //表格头部图标
        autoAddWidth:true,						//by:zhaiwenshan 作用：是否当列表所有列的总宽度不能将列表填满时，自动按照比例扩大列宽
        isPrompt:true,                           //by:hehua 作用：表格数据改变时，页面是否需要提示--'数据已经改变,如果继续将丢失数据,是否继续?'
        isAsync:false,								//by:zhangtao 作用:是否异步渲染数据
        bufferView:false,                        //
		bufferRenderNum:50
    };
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("                <div></div>");//增加空的div 否则可能出现不显示分页信息 houwei 20150325
        ...
        _setUrl: function (value)
        {
            ...
            {   
                ...
                this.options.newPage = 1;//  tianming 20120518  :解决按新条件查询不从第一页开始查的问题
                this.loadData(true);
            }
            else
            {
        ...
        loadData: function (loadDataParm)
        {
            ...
            {  
            	if(p.forceLocal){
	                ...
	                loadServer = false;
	               // p.dataType = "server"; 解决：本地数据加载不了的问题 tianming  20121219
	                //p.data = loadDataParm;
	                p.newPage = 1;
            	}
            }
            ...
            var param = [];
            //-------------------------------------------------------------------------------------
            if(loadDataParm&&loadDataParm.length){//查询参数不为空将其放入参数数组中  tianming 20120414
        ...
        loadServerData: function (param, clause)
        {
            ...
            var ajaxOptions = {
                ...
                success: function (data)
                {
                    ...
                    {
                        g.currentData = g.data = data;			// 直接赋值data不再赋空对象，因为ipage中存放了页面读取的除getResult和resultCount之外的其他属性 houwei 20130527
      ...
      //是否允许选择行
        ...
        */
        ...
        updateCol: function (arg,value,validationFunction)//validationFunction增加校验方法，当校验方法返回true是才更新当前列 houwei 20150318
        {
            var g = this, p = this.options;
...
//                return false;
        ...
        toggle: function (rowParm)
        {
            ...
            var children;//modify by zhaokun 20140305 gridTree展开树时仅仅展开下一层的子数据,而不是展开所有
            if (linkbtn.hasClass("l-grid-tree-link-close")) //收缩
            {
            	if(p.isAsync&&rowdata.isRender==undefined) {//展开时才渲染 add by zhangtao 20140403
            		rowdata.isRender = "Y";
            		...
            		var childHtml;
            		if (g.enabledFrozen()){
		            	childHtml = g._getHtmlFromData(g.rows,true,rowid);//固定列
		            	$(targetRowObj[1]).after($(childHtml));
            		}
	            	childHtml = g._getHtmlFromData(g.rows,false,rowid);//非固定列
        ...
        //根据行索引设置行css 田明 20121213 {backgroundColor:'',color:'',font:''}  manager.setGridRowCss(1,{color:'red',fontSize})
        ...
        _initColumns: function ()
        {
          ...
          //此方法解决当使用分辨率更大的屏幕时，指定固定列宽的表格无法填满表格的问题   by:zhaiwenshan  at:2012-11-9
        ...
        _getColumnWidth: function (column)
        {
            ...
            {//根据列标题的长度计算列宽并与指定的列宽进行比较，如指定列宽不足以显示标题则自动放宽（zhaiwenshan）
            	var temp_width = 0;
            ...
            if (typeof (colwidth) == "string" && colwidth=='auto-head'){//根据列标题的长度自动计算列宽（zhaiwenshan）
	            var headerText = "";
	            if (column.display && column.display != "")
	                headerText = column.display.split("<")[0];
	            else if (column.headerRender)
            	...
            	//根据制定列内容长度的最大值(单字节)，自动计算列宽，当计算结果无法完整显示标题时，将按照标题宽度显示（zhaiwenshan）
	            var headerText = "";
	            if (column.display && column.display != "")
	                headerText = column.display.split("<")[0];
	            else if (column.headerRender)
        ...
        _createHeaderCell: function (column)
        {
            ...
            if (column.valign) jcell.attr({ "valign": column.valign }); //增加垂直对齐方式  add by weichengcai 20130412 
        ...
        //调整固定列高度 tianming 20130125
        _setForzenHeight:function(){
        	var g = this, p = this.options;
        	if(p.rownumbers||p.isCheck){
            	  $("td[id$=c101]","#"+g.id).css("width",$(document.getElementById(g.id+"|hcell|c101")).width());//设置固定列行高度
        ...
        _getHtmlFromData: function (data, frozen, pid)
        {
            ...
            {   
            	...
            	if(!frozen&&p.bufferView){//启用缓存视图时
        ...
        _applyEditor: function (obj)
        {
            ...
            if (column.editor.type && p.editors[column.editor.type])
            {
                ...
                if(column.editor.isMoney){//add by zhangtao 20140318 增加editor是否金额属性
                ...
                g.bind('sysEndEdit', function ()
                {
                    ...
                    if (newValue != currentdata)
                    {
                        ...
                        //修改时间：2012-09-21 16:22:30   修改人：zhaiwenhsan  修改原因：原来的写法不便于验证修改后的信息
                        if (column.editor.onChange){//如果编辑器配置了这个属性，则执行自定义的方法，采用自定义方法的返回值作为单元格的值。
                        	editParm.value=column.editor.onChange(rowcell, newValue, editParm);
                        }else{//如果没有自定义的onchang，则直接赋值
        ...
        _getTotalCellContent: function (column, data)
        {
            ...
            if (column.totalSummary)
            {
                ...
                for (var i = 0; i < data.length; i++)
                {
                    ...
                    if (data[i].__status=='delete'||!value||data[i]._isTotalRow!=undefined) continue;//状态是删除的不参与汇总计算 tianming 20130608 长沙
				...
				function commafyNum(num,n){
					...
					if(num.indexOf("e")>0){ // 解决金额采用科学计数法表示-js中数据运算中的科学计数法使用的是小写字母e
						var temp = (num+"").split("e");
						var temp_amt = parseFloat(temp[0])*Math.pow(10,parseInt(temp[1]));
						num = temp_amt + "";
					}
        ...
        _getTotalSummaryHtml: function (data, classCssName, frozen)
        {
            ...
            $(g.columns).each(function (columnindex, column)
            {
                ...
                if (this.isrownumber)
                {
                    totalsummaryArr.push('<td class="l-grid-totalsummary-cell l-grid-totalsummary-cell-rownumbers" style="width:' + this.width + 'px"><div>合计</div></td>');
                    return;
                }
                //如果是复选框(系统列)
                if (this.ischeckbox)
        ...
        _onClick: function (e)
        {
            ...
            else if (src.row && g.enabledCheckbox()) //复选框选择行
            {
                ...
                if (src.checkbox || !selectRowButtonOnly)
                {
                    ...
                    g[met](src.data,src);//将src传递给select方法，从而传递给SelectRow事件，用于有多选框时，选中多选框了，但不查询这笔资产详情的事件 houwei 20140824
