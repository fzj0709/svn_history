...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        {   
            var g = this, p = this.options;
            if(p.defined!=undefined&&p.defined){
                ...
                $("div[tabid='"+tabid+"']",g.tab.content).parent().children().css("z-index","1");
                $("div[tabid='"+tabid+"']",g.tab.content).css("z-index","3");
        ...
        {
            var g = this, p = this.options;
            if(this.enabledClickToSelectTabItem()){
        ...
        //notabid 过滤掉tabid的
        //noclose 过滤掉没有关闭按钮的
        getTabidList: function (notabid, noclose,hidden)
        {
            var g = this, p = this.options;
            var tabidlist = [];
            var str = "";
            ...
            $("> li"+str, g.tab.links.ul).each(function ()
            {
                if ($(this).attr("tabid")
        ...
        reload: function (tabid,urlNew,text)
        {
            var g = this, p = this.options;
            ...
            var iframe = $("iframe:first", contentitem);
            var url = null;
            if(urlNew!=undefined&&urlNew!=null){//重新加载，如果传入了新的url则直接用传入的url  田明 20121215
                url = urlNew;
                $("li[tabid='"+tabid+"'] a").html(text);
            }else{
                url = $(iframe).attr("src");
            }
            iframeloading.show();
        ...
        enabledClickToSelectTabItem: function ()
        {
            return this.options.clickToSelectTabItem ? true : false;
        },
        /**
         * 隐藏标签页方法
         */
